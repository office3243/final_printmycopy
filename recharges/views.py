...
def create_with_offer_pack(request, offer_pack_id):
    ...
    recharge = Recharge.objects.create(wallet=request.user.get_wallet, pack=offer_pack, gateway=gateway)
    ...
    try:
        if request.method == "POST":
            ...
            if custom_price > 1.00:
                ...
                recharge = Recharge.objects.create(wallet=request.user.get_wallet, pack=custom_pack, gateway="PAYTM")
                return create_payment(request, recharge)
        return redirect("wallets:view")
    except:
        return redirect("wallets:view")
